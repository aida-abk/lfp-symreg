...
from filter.get_fixed_trials import fixation_trials, limit_trials, non_fixation_trials
...
def save_trial_filter_csv(data: TrialData, path: Path) -> None:
  ...
  with path.open("w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["trial", "is_excluded_fixation", "trial_rows", "channel0_samples"])
...
def run_single_fit(args, data: TrialData, trials: list[int]) -> None:
  ...
  print(f"non-fixation trials used: {len(trials)}")
  ...
  if args.save_npz:
    ...
    np.savez_compressed(
      ...
      excluded_fixation_trials=np.asarray(fixation_trials(data)),
...
def run_delay_sweep(args, data: TrialData, trials: list[int]) -> None:
  ...
  for n_delays in parse_int_list(args.n_delays_list):
    ...
    row = {
      "channel": args.channel,
      "excluded_fixation_trials": len(fixation_trials(data)),
      "included_non_fixation_trials": len(trials),
      "n_delays": n_delays,
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="Exclude likely fixation trials, delay-embed LFP, and fit PySINDy."
  )
  ...
  parser.add_argument(
    "--trial-filter-csv",
    type=Path,
    default=Path("outputs/pysindy/non_fixation_trial_filter.csv"),
  )
  parser.add_argument(
    "--out-csv",
    type=Path,
    default=Path("outputs/pysindy/non_fixation_delay_experiment_results.csv"),
  )
  args = parser.parse_args()

  data = TrialData.load(args.mat_file)
  fixation = fixation_trials(data)
  trials = limit_trials(non_fixation_trials(data), args.max_trials)
  save_trial_filter_csv(data, args.trial_filter_csv)

  print(f"total trials: {data.n_trials}")
  print(f"excluded likely fixation trials: {len(fixation)}")
  print(f"available non-fixation trials: {data.n_trials - len(fixation)}")
  print(f"using non-fixation trials: {len(trials)}")
